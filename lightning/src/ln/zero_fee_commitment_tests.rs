...
#[test]
fn test_counterparty_offered_htlc_claim_tx() {
	// Exercise the full `option_htlcs_claim_tx` offered-HTLC on-chain resolution path: a routed
	// HTLC is claimed by the recipient on-chain when the *counterparty's* commitment confirms,
	// producing the templated, zero-fee version 3 claim transaction, which is fee-bumped via a CPFP
	// child (a TRUC 1-parent-1-child package).
	//
	// Note: the test harness verifies spends with `bitcoinconsensus`, which does not implement
	// `OP_TEMPLATEHASH` (in tapscript `0xce` is `OP_SUCCESS206`, so the script-path spend succeeds
	// unconditionally). This therefore validates the taproot commitment and all of the LDK wiring,
	// but not the template-hash digest itself.
	let chanmon_cfgs = create_chanmon_cfgs(2);
	let node_cfgs = create_node_cfgs(2, &chanmon_cfgs);
	let mut user_cfg = test_default_channel_config();
	user_cfg.channel_handshake_config.our_htlc_minimum_msat = 1;
	user_cfg.channel_handshake_config.negotiate_anchor_zero_fee_commitments = true;
	user_cfg.channel_handshake_config.negotiate_htlcs_claim_tx = true;
	...
	// Route an HTLC from node 0 to node 1 and have node 1 learn the preimage, but don't deliver the
	// resulting messages, so that node 0's commitment keeps the offered HTLC output.
	const HTLC_AMT_MSAT: u64 = 1_000_000;
	let (preimage, payment_hash, _, _) = route_payment(&nodes[0], &[&nodes[1]], HTLC_AMT_MSAT);
	nodes[1].node.claim_funds(preimage);
	check_added_monitors(&nodes[1], 1);
	...
	// The counterparty's commitment confirms.
	mine_transaction(&nodes[0], &node_0_commit_tx[0]);
	mine_transaction(&nodes[1], &node_0_commit_tx[0]);
	...
	// Node 1 resolves the offered HTLC via the preimage path, yielding an HTLC-claim-tx CPFP event.
	let mut events = nodes[1].chain_monitor.chain_monitor.get_and_clear_pending_events();
	assert_eq!(events.len(), 1);
	let bump_event = match events.pop().unwrap() {
		Event::BumpTransaction(bump @ BumpTransactionEvent::HTLCsClaimTxResolution { .. }) => bump,
		ev => panic!("Unexpected event: {:?}", ev),
	};
	nodes[1].bump_tx_handler.handle_event(&bump_event);

	// The templated claim transaction and its fee-paying child are broadcast together as a package.
	let broadcasts = nodes[1].tx_broadcaster.txn_broadcast();
	assert_eq!(broadcasts.len(), 2);
	// The claim transaction is the fixed single-input, single-output version 3 transaction spending
	// the offered HTLC output on the counterparty's commitment. We can't use `check_spends!` here
	// because the claim transaction is intentionally zero-fee (it is fee-bumped by the child), but
	// we still run consensus verification of the taproot script-path spend.
	assert_eq!(broadcasts[0].version, bitcoin::transaction::Version::non_standard(3));
	assert_eq!(broadcasts[0].input.len(), 1);
	assert_eq!(broadcasts[0].output.len(), 1);
	let commit_txid = node_0_commit_tx[0].compute_txid();
	assert_eq!(broadcasts[0].input[0].previous_output.txid, commit_txid);
	broadcasts[0]
		.verify(|outpoint: &bitcoin::OutPoint| {
			if outpoint.txid == commit_txid {
				node_0_commit_tx[0].output.get(outpoint.vout as usize).cloned()
			} else {
				None
			}
		})
		.unwrap();
	// The child fee-bumps the claim transaction, spending its output plus a confirmed UTXO.
	assert_eq!(broadcasts[1].version, bitcoin::transaction::Version::non_standard(3));
	check_spends!(broadcasts[1], broadcasts[0], coinbase_tx);
}
